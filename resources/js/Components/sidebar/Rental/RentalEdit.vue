...
const props = defineProps<{
  rental: {
    ...
    items: Array<{
      ...
      product: { id: number; name: string; item_code: string; description: string | null; images?: Array<{ id: number; file_path: string; thumbnail_path: string | null; label: string | null; is_primary: boolean; sort_order: number }> }
    }>
  }
...
let itemKey = props.rental.items.length

const form = useForm({
  customer_id: props.rental.customer_id as number | null,
  customer: {
    first_name: props.rental.customer.first_name,
    last_name: props.rental.customer.last_name,
    contact_number: props.rental.customer.contact_number ?? '',
    address: props.rental.customer.address ?? '',
    affiliation: props.rental.customer.affiliation ?? '',
    social_media_link: props.rental.customer.social_media_link ?? '',
  },
  pickup_date: props.rental.pickup_date,
  return_date: props.rental.return_date,
  internal_notes: props.rental.internal_notes ?? '',
  items: props.rental.items.map(i => ({
    _key: itemKey++,
    id: i.id,
    product_id: i.product_id,
    description: '',
    images: [] as Array<{ id: string; file: File; previewUrl: string }>,
    rental_fee: parseFloat(i.rental_fee),
    security_deposit: parseFloat(i.security_deposit),
  })) as Array<{
    _key: number
    id?: number
...
const selectedCustomerName = ref(formatCustomerName(props.rental.customer.first_name, props.rental.customer.last_name))
...
const availableProducts = computed(() =>
  props.products.filter(p => p.status === 'available' || form.items.some(i => i.product_id === p.id))
)
...
const submit = () => {
  ...
  form.put(route('rentals.update', props.rental.id), {
...
<template>
  <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h2 class="text-lg font-semibold text-slate-800 dark:text-white">Edit Rental #{{ rental.id }}</h2>
    </div>
    ...
    <form @submit.prevent="submit" class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input v-model="form.pickup_date" type="date" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
        ...
        <div>
          ...
          <input v-model="form.return_date" type="date" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
        ...
        <div v-for="(item, index) in form.items" :key="item._key" class="border border-slate-200 dark:border-slate-700 rounded-lg p-3 mb-2 space-y-2">
          ...
          <div>
            ...
            <select v-model="item.product_id" @change="onProductSelect(index)" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
              ...
              <option v-for="p in availableProducts" :key="p.id" :value="p.id">{{ p.item_code }} — {{ p.name }}</option>
            </select>
          </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-if="props.rental.items.find(i => i.id === item.id)?.product.images?.length" class="flex flex-wrap gap-2 mb-2">
                <button v-for="(img, i) in props.rental.items.find(i => i.id === item.id)?.product.images" :key="img.id" type="button" @click="openPreview(props.rental.items.find(i => i.id === item.id)?.product.images?.map(x => '/storage/' + x.file_path) ?? [], i)" class="w-14 h-14 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-600 cursor-pointer hover:ring-2 hover:ring-blue-400 transition-shadow">
                  <img :src="'/storage/' + img.file_path" class="w-full h-full object-cover" />
                </button>
              </div>
              <div v-if="props.rental.items.find(i => i.id === item.id)?.product.description" class="text-xs text-slate-500 dark:text-slate-400">
                {{ props.rental.items.find(i => i.id === item.id)?.product.description }}
              </div>
      ...
      <div class="flex gap-2">
        <button type="submit" :disabled="form.processing" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50">
          {{ form.processing ? 'Updating...' : 'Update Rental' }}
        </button>
